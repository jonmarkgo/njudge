	...
	if (t == PLAGUE) {
		for (u = 1; u <= nunit; u++) {
			...
			if (is_infected(p = unit[u].loc)) {
				if (!d++)
					fputc('\n', rfp);
				fprintf(rfp, "%s %s%s in %s eliminated by famine.\n",
					owners[unit[u].owner],
...
/* Somehow the row and colums on EFF are reversed */
		...
		if (r || c) {
			...
			/*
			   **  Eliminate any unit in a plagued province.
			 */
			...
			if (t == PLAGUE) {
				...
				for (u = 1; u <= nunit; u++) {
					...
					if (is_infected(p = unit[u].loc)) {
						if (!d++)
							fputc('\n', rfp);
						fprintf(rfp, "%s %s%s in %s eliminated by plague.\n",
						   owners[unit[u].owner],
							Stype(unit[u].stype), Utype(unit[u].type),
							pr[p].name);
						unit[u].owner = 0;
