/* zparseb.c
   - Diplomacy Blind Parsing - blind code - Version 0.8
*/
...
{
  static char subNew[100]; // Maximum is the country adjective + 5or7

  // Note: here 'sub' is always uppercase with blanks before&after.
  //       e.g. ' ENGLISH '
  //
  ASSERTION(sub!=NULL && str!=NULL,"FindStringDislodged(1)");
  ASSERTION((str[0]>='a' && str[0]<='z')==false,"FindStringDislodged(2)");
  sprintf( subNew, "THE%s", sub );
  return FindString(subNew,str);
}
...
int ParseInput (ArrayResStr* lPtr, IoOptions* ioPtr)
{
  ...
  int nLenNon0;
  int loLimit, hiLimit;
  t_uchar lineBuffer[2048];  // More than enough for 70/80 chrs/line!
  t_uchar* str;
  t_uchar* pResultStr;
  unsigned pos, posTHE, nOccurr;
  bool isEmptyLine;
  bool lastDislAccepted = false;
  ...
  for (i=loLimit+1; i<hiLimit; i++) {
    str = lPtr->s[i];
    len = strlen(str);
    isEmptyLine = len==0;  // str is always right-trimmed!
    if ( isEmptyLine ) {
	...
	break;
      case e_Result_Dislodged:
	pos = FindStringDislodged(powerAdjBlankUpcase,lineBuffer);
	posTHE = FindStringDislodged(" ",lineBuffer);
	if ( (pos==0 || VarHideDislodged(ioPtr)==true) &&
	     ((posTHE!=1 && lastDislAccepted)==false ) ) {
	  str[0] = '?';
	  lastDislAccepted = false;
	}
	else {
	  countDislodged++;
	  // Set a flag (lastDislAccepted) to remember last
	  // line was accepted in dislodgement!
	  lastDislAccepted = true;
	}
#ifdef DEBUG_VAR
	fprintf(fDebug,"PARSE: Dislodged [%s] (pos=%u,count=%d) THE?%c LASTDISL?%c\n",
		str, pos, countDislodged,
		isLastTHE?'Y':'N', lastDislAccepted?'Y':'N');
#endif //DEBUG_VAR
	break;
      case e_Result_Ownership:
	// Care must be taken in this case in order to
	...
	case '.':
	  ...
	  if ( wasLastLineCut && lastChrLineBefore==',' )
	    str[0] = '@';
	  // Currently zpblind input may _NOT_ have '...Moscow, St'
	  // as the lastStr.
	  // The following code is not necessary:
	  //  ==>  if ( wasLastLineCut && lastChrLineBefore!='.' ) str[0] = '@';
	  break;
	case ',':
	  // E.g.:
	  //   @ustria:  Moscow, St Petersburg,
